  ...
  // Auto-detect or download android.jar
  if (!androidJarPath) {
    const androidHome = process.env.ANDROID_HOME || process.env.ANDROID_SDK_ROOT
    if (androidHome) {
      // Find highest API level locally
    ...
    if (!androidJarPath || !existsSync(androidJarPath)) {
      console.log('Android SDK not found locally. Preparing to download android.jar...')
      const cacheDir = join(homedir(), '.nitron', 'android')
      mkdirSync(cacheDir, { recursive: true })
      androidJarPath = join(cacheDir, 'android.jar')
      ...
      if (!existsSync(androidJarPath)) {
        ...
        if (!jarEntry) {
          throw new Error('android.jar not found in downloaded zip')
        }
  ...
  if (!androidJarPath || !existsSync(androidJarPath)) {
    console.error('Error: Cannot find or download android.jar')
    process.exit(1)
  }
